...
class CharterState extends FunkinState {
	...
	override public function create() {
		...
		for (note in chart.notes) {
			var targetNote:ChartNote = note.copy();
			var strumline:CharterStrumline = strumlines.members[targetNote.laneIndex];
			strumline?.queueNote(targetNote, null, false, false);
		}
	...
	override public function update(elapsed:Float) {
		...
		if (FlxG.mouse.justReleased) {
			if (pickedNote != null) {
				...
				final laneDiff:Int = laneToIndex(pickedNote.laneIndex, pickedNote.strumlineIndex) - pickedLaneIndex;
	...
	public function queueNote(note:ChartNote):ChartNote {
		var strumline:Strumline = strumlines.members[note.strumlineIndex];
		strumline?.queueNote(note);
		strumline?.updateQueue();
		return note;
	}
	public function spawnNote(note:ChartNote):CharterNote {
		var strumline:Strumline = strumlines.members[note.strumlineIndex];
		var lane:Lane = strumline?.getNoteLane(note);
		var note:CharterNote = null;
		if (lane != null) {
			note = lane.insertNote(note);
			strumline?.updateQueue();
		}
		return note;
	}
	...
	public function twistNotes(notesArray:Array<CharterNote>, laneMod:Int = 0):Int {
		...
		var maxLane:Int = -1;
		for (note in notesArray) {
			var laneIdx:Int = laneToIndex(note.laneIndex, note.strumlineIndex);
	...
	public function shiftNote(note:CharterNote, beatTime:Float) {
		if (note == null) {
			Log.warning('shiftNote: ???');
			return;
		}
		
		note.msTime = conductorInUse.convertMeasure(beatTime, BEAT, MS);
		note.updateChartNote();
	}
	public function twistNote(note:CharterNote, lane:Lane) {
		...
		note.strumlineIndex = getStrumlineIndex(findLaneStrumline(lane));
		note.laneIndex = lane.laneIndex;
		
		note.lane.killNote(note);
		queueNote(note.chartNote);
		
		note.lane = lane;
	}
	public function getStrumlineIndex(strumline:CharterStrumline):Int
		return strumlines.members.indexOf(strumline);
	public function findLaneStrumline(lane:Lane):Int {
		for (strumline in strumlines) {
			var laneIdx:Int = strumline.lanes.indexOf(lane);
			if (laneIdx != -1) return laneIdx;
		}
		return -1;
	}
	...
	public function laneToIndex(laneIndex:Int, strumlineIndex:Int) {
		var n:Int = 0;
		for (i => strumline in strumlines) {
			if (i == strumlineIndex) return n + laneIndex;
			n += strumline.laneCount;
		}
		return -1;
	}
	...
	public function keyPressEvent(event:KeyboardEvent) {
		...
		switch (key) {
			case FlxKey.DELETE:
				...
				for (note in getSelectedNotes()) {
					deletedNotes.push(note.chartNote);
	...
	public function undo() {
		...
		if (undoAction != null) {
			...
			switch (undoAction.type) {
				...
				case REMOVED_NOTES:
					for (note in undoAction.notes) {
						var note:CharterNote = spawnNote(note);
						note.selected = true;
					}
				case SHIFTED_NOTES:
	...
	public function copySelectedNotes() {
		...
		if (selectedNotes.length > 0) {
			...
			for (note in selectedNotes)
				copiedNotes.push(note.chartNote);
			...
			case FlxKey.V: // PASTE
				...
				if (copiedNotes.length > 0) {
					...
					for (note in generatedNotes) {
						var note:Note = generateNote(note.chartNote);
						
						note.beatTime += beatDiff;
						note.updateChartNote();
						note.justCopied = true;
						note.selected = true;
					}
...
class CharterStrumline extends Strumline {
	...
	public function new(laneCount:Int = 4, direction:Float = 90, scrollSpeed:Float = 1) {
		super(laneCount, direction, scrollSpeed, CharterNote);
...
class CharterNote extends Note {
	...
	public override function set_kind(newKind:String) {
		if (kind == newKind) return newKind;
	...
	public function actuallyDraw() {
		...
		pointer.x = x;
		pointer.y = y;
		if (noteKind != '' && highlighted && CharterState.inEditor && !FlxG.mouse.pressed) {
			pointer.x += width * .7;
			pointer.y += height * .3;
			var charter:CharterState = CharterState.instance;
