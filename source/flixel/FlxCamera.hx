...
class FlxCamera extends FlxBasic
{
	...
	inline function rotateMatrix(mat:FlxMatrix):Void {
		if (this._angleRad == 0) return;
		
		mat.translate(-this.width * .5, -this.height * .5);
		mat.rotate(this._angleRad);
		mat.translate(this.width * .5, this.height * .5);
		
		return;
	}
	public function drawPixels(?frame:FlxFrame, ?pixels:BitmapData, mat:FlxMatrix, ?transform:ColorTransform, ?blend:BlendMode, ?smoothing:Bool = false, ?shader:FlxShader):Void {
		if (FlxG.renderBlit) {
			_helperMatrix.copyFrom(mat);
			...
			#end
			drawItem.addQuad(frame, mat, transform);
		}
	}
	...
	public function drawTriangles(graphic:FlxGraphic, vertices:DrawData<Float>, indices:DrawData<Int>, uvtData:DrawData<Float>, ?colors:DrawData<Int>, ?position:FlxPoint, ?blend:BlendMode, repeat:Bool = false, smoothing:Bool = false, ?transform:ColorTransform, ?shader:FlxShader):Void {
		if (FlxG.renderBlit) {
			...
			if (!_bounds.overlaps(bounds)) {
				...
				FlxCamera.trianglesSprite.graphics.drawTriangles(FlxCamera.drawVertices, indices, uvtData);
	...
	public function getViewCenter(?point:FlxPoint):FlxPoint {
		if (point == null)
			point = FlxPoint.get();
		
		return point.set((viewMarginLeft + viewMarginRight) * .5, (viewMarginTop + viewMarginBottom) * .5);
	}
	
	// getViewMarginRect, but honors rotation
	public function getRotatedViewMarginRect(?rect:FlxRect):FlxRect {
		var rect:FlxRect = getViewMarginRect(rect);
		if (_angleRad == 0)
			return rect;
		
		return rect.getRotatedBounds(rotation, getViewCenter(_point), rect);
	}
	...
	{
		var rotatedView:FlxRect = getRotatedViewMarginRect(_rect);
		var contained = (point.x + width > rotatedView.left) && (point.x < rotatedView.right)
			&& (point.y + height > rotatedView.top) && (point.y < rotatedView.bottom);
		point.putWeak();
		return contained;
	}
	...
	{
		var rotatedView:FlxRect = getRotatedViewMarginRect(_rect);
		var contained = (rect.right > rotatedView.left) && (rect.x < rotatedView.right)
			&& (rect.bottom > rotatedView.top) && (rect.y < rotatedView.bottom);
		rect.putWeak();
		return contained;
	}
	
	function set_rotation(newAngle:Float) {
		_angleRad = (newAngle % 360) * flixel.math.FlxAngle.TO_RAD;
